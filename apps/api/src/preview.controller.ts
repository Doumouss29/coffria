  ...
  async preview(@Req() req: any, @Param('id') id: string) {
    ...
    if (['docx','doc','xlsx','xls','pptx','ppt','odt','ods','odp'].includes(ext)) {
      try {
        ...
        return { ...common, kind: 'pdf', office: true, url: await this.storage.downloadUrl(previewKey, 'inline'), downloadUrl: await this.storage.downloadUrl(doc.storageKey, 'attachment') };
      } catch (error) {
