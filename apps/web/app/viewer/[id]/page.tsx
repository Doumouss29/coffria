...
import { ArrowLeft, Download, FileText, Maximize2, Minus, Plus, RefreshCcw } from 'lucide-react';
...
export default function ViewerPage(){
  ...
  return <AppShell title="Visionneuse documentaire"><section className="viewerPage">
    <div className="viewerToolbar"><Link href="/explorer" className="secondary"><ArrowLeft size={16}/> Retour</Link><div className="viewerTitle"><strong>{data.name}</strong><span>{(data.extension||data.mimeType||'fichier').toUpperCase()}</span></div>{data.downloadUrl&&<a className="secondary" href={data.downloadUrl}><Download size={16}/> Télécharger</a>}</div>
    <div className="viewerCanvas">
      {data.kind==='pdf'&&<iframe title={data.name} src={data.url} className="documentFrame"/>}
