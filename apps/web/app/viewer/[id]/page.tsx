...
function PdfPage({pdf,pageNumber,onVisible}:{pdf:any;pageNumber:number;onVisible:(page:number)=>void}){
  ...
  useEffect(()=>{
    ...
    const observer=new IntersectionObserver((entries)=>{if(entries.some(e=>e.isIntersecting))onVisible(pageNumber)},{threshold:.55});observer.observe(host);return()=>observer.disconnect();
  },[pageNumber,onVisible]);
  useEffect(()=>{
    let cancelled=false;let renderTask:any;let resizeTimer:any;
    async function render(){
      const host=hostRef.current,canvas=canvasRef.current;if(!host||!canvas)return;setBusy(true);
      try{
        const page=await pdf.getPage(pageNumber);if(cancelled)return;
        const base=page.getViewport({scale:1});const width=Math.max(250,Math.min(1200,host.clientWidth-12));const viewport=page.getViewport({scale:width/base.width});
        canvas.width=Math.ceil(viewport.width);canvas.height=Math.ceil(viewport.height);canvas.style.width=`${Math.floor(width)}px`;canvas.style.height='auto';
        const ctx=canvas.getContext('2d');if(!ctx)return;renderTask=page.render({canvasContext:ctx,viewport});await renderTask.promise;
      }catch(e:any){if(!cancelled&&e?.name!=='RenderingCancelledException')console.error('PDF render error',e);}finally{if(!cancelled)setBusy(false)}
    }
    void render();
    const ro=new ResizeObserver(()=>{clearTimeout(resizeTimer);resizeTimer=setTimeout(()=>{try{renderTask?.cancel()}catch{};void render();},120)});if(hostRef.current)ro.observe(hostRef.current);
    return()=>{cancelled=true;clearTimeout(resizeTimer);ro.disconnect();try{renderTask?.cancel()}catch{}};
  },[pdf,pageNumber]);
  return <div className="pdfContinuousPage" ref={hostRef} data-page={pageNumber}><div className="pdfPageLabel">Page {pageNumber}</div><canvas ref={canvasRef} className="pdfDocumentCanvas"/>{busy&&<div className="pdfRendering">Chargement de la page…</div>}</div>;
}
...
export default function ViewerPage(){
  ...
  const[pdf,setPdf]=useState<any>(null);const[pdfPage,setPdfPage]=useState(1);const[pdfCount,setPdfCount]=useState(1);const pageRefs=useRef<Record<number,HTMLDivElement|null>>({});
  ...
  useEffect(()=>{
    if(data?.kind!=='pdf'||!data?.url)return;let cancelled=false;const task=pdfjsLib.getDocument({url:data.url});
  ...
  function goPage(page:number){const n=Math.max(1,Math.min(pdfCount,page));setPdfPage(n);pageRefs.current[n]?.scrollIntoView({behavior:'smooth',block:'start'});}
  ...
  return <AppShell title="Visionneuse documentaire"><section className="viewerPage">
    ...
    <div className="viewerCanvas">
      {data.kind==='pdf'&&<div className="pdfViewer"><div className="pdfPageTools"><button type="button" onClick={()=>goPage(pdfPage-1)} disabled={pdfPage<=1}><ChevronLeft size={18}/> <span>Précédente</span></button><strong>Page {pdfPage} / {pdfCount}</strong><button type="button" onClick={()=>goPage(pdfPage+1)} disabled={pdfPage>=pdfCount}><span>Suivante</span> <ChevronRight size={18}/></button></div><div className="pdfContinuousList">{pdf&&Array.from({length:pdfCount},(_,i)=>i+1).map(n=><div key={n} ref={el=>{pageRefs.current[n]=el}}><PdfPage pdf={pdf} pageNumber={n} onVisible={setPdfPage}/></div>)}</div></div>}
